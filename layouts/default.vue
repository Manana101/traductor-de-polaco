<template>
  <v-app dark>
    <v-toolbar app>
      ...
      <v-menu offset-y min-width="100%" v-if="$vuetify.breakpoint.smAndDown">
        ...
        <v-list>
          <v-list-tile
            ...
            exact
          >
            <v-list-tile-title class="text-uppercase text-truncate">
              {{ item.title }}
            </v-list-tile-title>
          </v-list-tile>
        </v-list>
      ...
      <v-toolbar-items v-if="$vuetify.breakpoint.mdAndUp">
        <template v-for="item in navItems">
          <v-btn flat :key="item.title" :to="item.to" exact>{{ item.title }}</v-btn>
        </template>
      </v-toolbar-items>
    </v-toolbar>
    ...
    <v-footer app height="64px" class="pr-4 pl-4">
      <v-spacer></v-spacer>
      <v-btn outline href="mailto:olga@traductordepolaco.eu">Contacto</v-btn>
      <linked-in-link />
      <facebook-link />
    </v-footer>
...
export default {
  ...
  data() {
    return {
      ...
      title: 'TRADUCTOR E INTÉRPRETE DE POLACO',
      navItems: [
        {
          title: 'Inicio',
          to: '/'
